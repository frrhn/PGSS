// ==========================================
// PHARMAFIELD COMMAND CENTER - FINAL PRODUCTION
// ==========================================
...
function showTab(id, btn) {
    ...
    if (id === 'dashboard' && map) setTimeout(() => map.invalidateSize(), 100); // Fix map rendering glitch
}
...
function toggleTheme() {
    ...
    // Redraw charts with new theme colors if analytics is open
    if (document.getElementById('analytics').style.display !== 'none') loadAnalytics();
}
...
async function loadDashboard() {
    ...
    if (error) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:20px; color:var(--red);">Error: ${error.message}</td></tr>`;
        return;
    }
    ...
    data.forEach(r => {
        ...
        let mediaHtml = photo ? `<img src="${photo}" style="width:40px; height:40px; object-fit:cover; border-radius:6px; border:1px solid var(--border);">` : '<span style="color:var(--text2);">--</span>';
        if (r.audio_url) mediaHtml += ' 🎵';
...
async function exportCSV() {
    ...
    const headers = ['Agent ID', 'Timestamp', 'Latitude', 'Longitude', 'Compliance', 'Distance (m)', 'Photo URL', 'Audio URL'];
    const rows = data.map(r => [r.agent_id, r.timestamp, r.latitude, r.longitude, r.compliance_status, r.distance_meters, r.photo_url, r.audio_url]);
...
async function loadAnalytics() {
    ...
    complianceChartInstance = new Chart(document.getElementById('complianceChart'), {
        type: 'doughnut',
        data: { labels: ['Compliant', 'Out of Bounds', 'Auto-Logs', 'Unknown'], datasets: [{ data: [compliant, oob, auto, unknown], backgroundColor: ['#10B981', '#EF4444', '#F59E0B', '#64748B'], borderWidth: 0 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Geofence Compliance Rate', color: textColor }, legend: { labels: { color: textColor } } } }
    });

    // Bar Chart
    const dailyCounts = {};
    data.forEach(r => { const day = new Date(r.timestamp).toLocaleDateString(); dailyCounts[day] = (dailyCounts[day] || 0) + 1; });

    activityChartInstance = new Chart(document.getElementById('activityChart'), {
        type: 'bar',
        data: { labels: Object.keys(dailyCounts), datasets: [{ label: 'Daily Check-ins', data: Object.values(dailyCounts), backgroundColor: '#00E5FF', borderRadius: 6 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Field Activity Trends', color: textColor }, legend: { labels: { color: textColor } } }, scales: { y: { beginAtZero: true, ticks: { color: textColor, stepSize: 1 }, grid: { color: gridColor } }, x: { ticks: { color: textColor }, grid: { color: gridColor } } } }
    });
}
